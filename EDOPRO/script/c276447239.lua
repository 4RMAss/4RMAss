...
function s.initial_effect(c)
    ...
    -- Efecto para invocar monstruos movidos cuando un monstruo es destruido por batalla y no controlas monstruos (obligatorio)
    ...
    -- Efecto para invocar monstruos movidos cuando un monstruo deja el campo por el efecto de una carta y no controlas monstruos (obligatorio)
    ...
    -- Proteger la activación de la carta de ser negada
    ...
    -- Destruir monstruos Fusión con materiales específicos y invocar/setear los materiales
    ...
    e11:SetCountLimit(1)
    e11:SetCondition(s.fuscon)
    e11:SetOperation(s.fusop)
...
function s.effectfilter(e,ct)
    ...
    return te:GetHandler():IsCode(id)
end

-- Condición para destruir monstruos Fusión con materiales específicos
function s.fuscon(e,tp,eg,ep,ev,re,r,rp)
    return Duel.IsExistingMatchingCard(s.fusfilter,tp,LOCATION_MZONE,0,1,nil)
end

-- Filtro para encontrar monstruos Fusión con materiales específicos
function s.fusfilter(c)
    return c:IsType(TYPE_FUSION) and c:IsFaceup() and (c:ListsCode(1784686) or c:ListsCode(1784687) or c:ListsCode(1784688)) -- Los códigos deben ser de "The Eye of Timaeus", "The Claw of Hermos", y "The Fang of Critias"
end

-- Operación para destruir monstruos Fusión y setear/invocar los materiales
function s.fusop(e,tp,eg,ep,ev,re,r,rp)
    local g=Duel.GetMatchingGroup(s.fusfilter,tp,LOCATION_MZONE,0,nil)
    for tc in aux.Next(g) do
        local mat=tc:GetMaterial()
        if Duel.Destroy(tc,REASON_EFFECT)~=0 and #mat>0 then
            for mc in aux.Next(mat) do
                if mc:IsType(TYPE_MONSTER) then
                    Duel.SpecialSummon(mc,0,tp,tp,false,false,POS_FACEUP)
                elseif mc:IsType(TYPE_TRAP) then
                    Duel.SSet(tp,mc)
                end
            end
        end
